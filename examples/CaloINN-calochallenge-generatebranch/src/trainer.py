...
class Trainer:
    ...
    def generate_Einc_ds1(self, energy=None, sample_multiplier=1000, pions=False):
        ...
        """
        if pions:
            ret = np.logspace(8,17,10, base=2)
            ret = np.tile(ret, 100)
            ret = np.array([*ret, *np.tile(2.**18, 98), *np.tile(2.**19, 50), *np.tile(2.**20, 30), *np.tile(2.**21, 20), *np.tile(2.**22, 10)])
            ret = np.tile(ret, sample_multiplier//10)
        else:
    ...
    def generate(self, num_samples, batch_size = 1000):
        ...
        with torch.no_grad():
            ...
            elif self.params.get('eval_dataset') == "1-pions":
                energies = (torch.tensor(self.generate_Einc_ds1(energy=self.single_energy, sample_multiplier=1000, pions=True), dtype=torch.float)/1e3).reshape(-1, 1)
            else:
