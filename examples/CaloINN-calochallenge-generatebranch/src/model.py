...
class CINN(nn.Module):
    ...
    def __init__(self, params, data, cond):
        ...
        self.bit_split = params.get("bit_split", False)
    ...
    def get_coupling_block(self, params):
        ...
        permute_switch = params.get("permute_switch", False)
    ...
    def define_model_architecture(self, in_dim):
        ...
        self.params["permute_switch"] = True
        sub_layers_backup = self.params["sub_layers"]
        for idx, entry in enumerate(self.params["sub_layers"]):
            self.params["sub_layers"][idx] = entry.replace("bit","")
        CouplingBlock_outer, block_kwargs_outer = self.get_coupling_block(self.params)
        self.params["permute_switch"] = False
        self.params["sub_layers"] = sub_layers_backup
        ...
        if self.bit_split:
            ...
            for i in range(self.params.get("n_blocks", 10)-4):
                if self.params.get("norm", True):
                    nodes.append(
                        ff.Node(
                            [nodes[-1].out0],
                            fm.ActNorm,
            ...
            for i in range(2):
                nodes.append(
                    ff.Node(
                        ...
                        block_kwargs_outer,
                        conditions = cond_node,
                        name = f"block_{self.params.get('n_blocks', 10)-2+i}"
                )
            )
        else:
