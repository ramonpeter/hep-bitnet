...
class PointCloudDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        return (
            self.data[index],
            self.labels[index],
            self.interact1[index],
            self.interact2[index],
        )
...
class EdgeConv1(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = torch.nn.Conv2d(
            out_channels, out_channels, kernel_size=(1, 1), bias=False
        )
    ...
    def forward(self, x):
        ...
        x = self.conv1(
            x.permute(0, 3, 1, 2)
        )  # (batch_size, out_channels, num_points, 11)
        x = self.bn1(x)
        x = self.gelu(x)
        x = self.conv2(x)
        x = self.bn2(x)
...
class EdgeConv2(nn.Module):
    ...
    def forward(self, x):
        ...
        pairwise_distances = torch.sum(
            (x.unsqueeze(2) - x.unsqueeze(1)) ** 2, dim=-1
        )  # (batch_size, num_points, num_points)
        _, indices = torch.topk(
            pairwise_distances, k=self.num_neighbors, dim=-1, largest=False
        )  # Nearest neighbor indices

        neighbor_features = x.unsqueeze(2).expand(
            -1, -1, num_points, -1
        )  # (batch_size, num_points, num_points, num_features)
        neighbor_features = torch.gather(
            neighbor_features, 2, indices.unsqueeze(-1).expand(-1, -1, -1, num_features)
        )

        x = self.conv1(
            neighbor_features.permute(0, 3, 1, 2)
        )  # (batch_size, out_channels, num_points, num_neighbors)
        x = self.bn1(x)
        x = self.gelu(x)
        x = self.conv2(x)
        x = self.bn2(x)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x, interact2):
        ...
        scores = torch.bmm(q, k.transpose(-2, -1)) / torch.sqrt(
...
for i in range(1, 1281):
    file_path = (
        f"/groups/hephy/mlearning/daohan/qg/new/new/train_data_with_neighbors{i}.npz"
    )
...
for i in range(1, 161):
    file_path = (
        f"/groups/hephy/mlearning/daohan/qg/new/new/valid_data_with_neighbors{i}.npz"
    )
...
for epoch in range(100):
    ...
    for i in range(1280):
        ...
        train_interact1 = np.load(
            "/groups/hephy/mlearning/daohan/qg/new/new/train_interact"
            + str(i + 1)
            + ".npz",
            mmap_mode="r",
        )["interact1"]
        train_interact2 = np.load(
            "/groups/hephy/mlearning/daohan/qg/new/new/train_interact_"
            + str(i + 1)
            + ".npy",
            mmap_mode="r",
        )
        train_dataset = PointCloudDataset(
            train_data, train_labels, train_interact1, train_interact2
        )
    ...
    torch.save(
        model.state_dict(),
        "/users/daohan.wang/DAT/qgc/3/weights/model-%.3d.weights" % epoch,
    )
    scheduler.step()
    ...
    for j in range(160):
        ...
        valid_interact1 = np.load(
            "/groups/hephy/mlearning/daohan/qg/new/new/valid_interact"
            + str(j + 1)
            + ".npz",
            mmap_mode="r",
        )["interact1"]
        valid_interact2 = np.load(
            "/groups/hephy/mlearning/daohan/qg/new/new/valid_interact_"
            + str(j + 1)
            + ".npy",
            mmap_mode="r",
        )
        valid_dataset = PointCloudDataset(
            valid_data, valid_labels, valid_interact1, valid_interact2
        )
