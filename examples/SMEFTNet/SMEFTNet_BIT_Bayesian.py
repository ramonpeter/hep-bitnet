...
from vblinear import VBLinearWithVariance
...
class VBBitLinear(VBLinear):
    ...
    def __init__(
        ...
        super().__init__(in_features, out_features, bias)
        self.eps = 1e-8
        self.device = self.mu_w.device
        self.dtype = self.mu_w.dtype
...
class VBBitLinearX(VBLinearWithVariance):
    """
    Bayesian version of the BitLinear layer
    ...
    def forward(self, x: Tensor) -> tuple:
        ...
        Returns:
            Tuple: (mean_output, var_output) during evaluation,
                   (mu_out, log_sigma2_out) during training.
        """
        # Quantize input
        x_quant = x + (self.quantize_activations(x) - x).detach()

        if self.training:
            # Training mode: use local reparameterization trick
            mu_w = self.mu_w
            logsig2_w = self.logsig2_w.clamp(-11, 11)
            s2_w = logsig2_w.exp()
            ...
            # Perform linear transformation
            mu_out = F.linear(x_quant, w_quant, self.bias) + 1e-8
            # Perform linear transformation for variance (log variance)
            sigma2_out = F.linear(x_quant.pow(2), w_quant, self.bias) + 1e-8
            log_sigma2_out = torch.log(sigma2_out)

            # Return dequantized output
            return mu_out, log_sigma2_out

        else:
            # Evaluation mode: perform 30 samples and return mean and variance
            outputs = []
            logsig2_w = self.logsig2_w.clamp(-11, 11)
            s2_w = logsig2_w.exp()

            for _ in range(30):  # Perform 30 samples
                random_sample = torch.randn_like(self.logsig2_w)
                w = self.mu_w + s2_w.sqrt() * random_sample
                w_quant = w + (self.quantize_weights(w) - w).detach()

                # Perform linear transformation for each sampled weight
                output = F.linear(x_quant, w_quant, self.bias) + 1e-8
                outputs.append(output)

            outputs = torch.stack(outputs)  # Stack all outputs to compute mean and variance
            mean_output = outputs.mean(dim=0)
            var_output = outputs.var(dim=0)  # Variance along the sample axis            
            log_sigma2_out = torch.log(var_output)
            
            return mean_output, log_sigma2_out  # Return mean and variance of the sampled outputs

      
class VBBitLinear158bX(VBBitLinearX):
    """
    Bayesian version of BitLinear158b layer
...
class CMLP(nn.Module):
    def __init__(self, layer_sizes, dropout=0.0, activation='ReLU', batch_norm=False):
        ...
        for i in range(len(layer_sizes) - 1):
            self.layers.append(VBBitLinear158b(layer_sizes[i], layer_sizes[i + 1]))
...
class CMLPX(nn.Module):
    def __init__(self, layer_sizes, dropout=0.0, activation='ReLU', batch_norm=False):
        super(CMLPX, self).__init__()
        self.layers = nn.ModuleList()
        ...
        # 构建最后一层，使用 VBBitLinear158bX，输出均值和方差
        self.layers.append(VBBitLinear158bX(layer_sizes[-2], layer_sizes[-1]))

    def forward(self, x):
        for layer in self.layers[:-1]:
            x = layer(x)
        # 最后一层输出均值和方差
        mu, sigma2 = self.layers[-1](x)
        return mu, sigma2        
...
class SMEFTNet(torch.nn.Module):
    def __init__(self, 
        ...
        for l, (dropout, hidden_layers) in enumerate(conv_params):
            ...
            if l==0:
                _mlp = CMLP([3*(1+num_features) + 2 ]+hidden_layers_, dropout=dropout, activation="leakyrelu")
                ...
                _mlp = CMLP([3*conv_params[l-1][1][-1]+2]+hidden_layers_,dropout=dropout, activation="leakyrelu")
                #_mlp.act.negative_slope = negative_slope
        ...
        self.mlp = CMLPX( [EC_out_chn+self.num_scalar_features]+readout_params[1]+[num_classes], dropout=readout_params[0], activation="leakyrelu",batch_norm=self.readout_batch_norm)
    ...
    def forward(self, pt, angles, features=None, scalar_features=None, message_logging=False, return_EIRCGNN_output=False):
        ...
        if len(self.EC)>0:
            if self.learn_from_gamma == True:
                if self.regression: 
                    return torch.cat( (self.mlp(x)[0], x[:, -2:]), dim=1)
                else:
                    return torch.cat( (self.out(self.mlp(x)[0]), x[:, -2:]), dim=1)
            else:
                if self.regression: 
                    return torch.cat( (self.mlp(x[:, :-2])[0], x[:, -2:]), dim=1)
                else:
                    return torch.cat( (self.out(self.mlp(x[:, :-2])[0]), x[:, -2:]), dim=1)
        else:
            if self.learn_from_gamma == True:
                raise RuntimeError( "No EC layer, can't learn from gamma!" )           
            else:
                if self.regression: 
                    return self.mlp(x)[0].view(-1,1)
                else:
                    return self.out(self.mlp(x[:, :-2])[0]).view(-1,1)
