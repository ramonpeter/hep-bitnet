...
class VBBitLinear(VBLinear):
    ...
    def forward(self, inpt):
        ...
        logsig2_w = self.logsig2_w.clamp(-10, 10)
        s2_w = logsig2_w.exp()
...
class VBBitLinearX(VBLinearWithVariance):
    ...
    def forward(self, x: Tensor) -> tuple:
        ...
        if self.training:
            ...
            mu_out = F.linear(x, w, self.bias) + 1e-8
            # Perform linear transformation for variance (log variance)
            sigma2_out = F.linear(x.pow(2), w, self.bias) + 1e-8
            ...
            # Evaluation mode: perform 20 samples and return mean and variance
            outputs = []
            logsig2_w = self.logsig2_w.clamp(-10, 10)
            s2_w = logsig2_w.exp()

            for _ in range(20):  # Perform 20 samples
                ...
                output = F.linear(x, w, self.bias) + 1e-8
...
class CMLP(nn.Module):
    def __init__(self, layer_sizes, dropout=0.0, activation='ReLU', batch_norm=False):
        ...
        for i in range(len(layer_sizes) - 1):
            self.layers.append(VBBitLinear(layer_sizes[i], layer_sizes[i + 1]))
...
class CMLPX(nn.Module):
    def __init__(self, layer_sizes, dropout=0.0, activation='ReLU', batch_norm=False):
        ...
        self.layers.append(VBBitLinearX(layer_sizes[-2], layer_sizes[-1]))
