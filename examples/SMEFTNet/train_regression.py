...
def print_params_and_grads(model, message=""):
    ...
    for name, param in model.named_parameters():
        print(
            f"{name} - Param: {param.data.mean().item()}, Grad: {param.grad.mean().item() if param.grad is not None else None}"
        )
        print("\n")
...
# model_directory = os.path.join( user.model_directory, 'SMEFTNet-Bit',  args.config, args.prefix)
...
model.cfg_dict.update(
    {key: getattr(args, key) for key in ["prefix", "learning_rate", "epochs"]}
)
model.cfg_dict.update(
    {
        "dRN": args.dRN,
...
for epoch in range(epoch_min, args.epochs):
    ...
    for i_data, data in enumerate(data_list):
        pt, angles, features, scalar_features, weights, truth = (
            config.data_model.getEvents(data)
        )
        if args.clip is not None:
            len_before = len(pt)
            # selection = helpers.clip_quantile( config.data_model.getScalarFeatures( data ), args.clip, return_selection = True )
            selection = helpers.clip_quantile(
                truth.view(-1, 1), args.clip, return_selection=True
            )
            pt = pt[selection]
            angles = angles[selection]
            features = features[selection] if features is not None else None
            scalar_features = (
                scalar_features[selection] if scalar_features is not None else None
            )
        ...
        loss = config.loss(
            out, truth[train_mask], weights[train_mask] if weights is not None else None
        )
        n_samples += len(out)
        ...
        with torch.no_grad():
            ...
            print(
                "Data %i/%i Loss train/test = %4.3f/%4.3f (Total: %4.3f/%4.3f)"
                % (
                    i_data,
                    len(config.data_model.data_generator),
                    loss.item(),
                    loss_test.item(),
                    model.cfg_dict["train_losses"][-1],
                    model.cfg_dict["test_losses"][-1],
                ),
                end="\r",
            )
    ...
    print(
        f"Epoch {epoch:03d} with N={n_samples:03d}, Loss(train): {loss:.4f} Loss(test, scaled to nTrain): {loss_test:.4f}"
    )
    ...
    if args.prefix:
        if loss_test.item() < model.cfg_dict["best_loss_test"]:
            ...
            torch.save(
                model.state_dict(), os.path.join(model_directory, "best_state.pt")
            )
            torch.save(
                optimizer.state_dict(),
                os.path.join(model_directory, "best_optimizer.pt"),
            )
            pickle.dump(
                model.cfg_dict,
                open(os.path.join(model_directory, "best_cfg_dict.pkl"), "wb"),
            )

        torch.save(
            model.state_dict(),
            os.path.join(model_directory, "epoch-%d_state.pt" % epoch),
        )
        torch.save(
            optimizer.state_dict(),
            os.path.join(model_directory, "epoch-%d_optimizer.pt" % epoch),
        )
        pickle.dump(
            model.cfg_dict,
            open(os.path.join(model_directory, "epoch-%d_cfg_dict.pkl" % epoch), "wb"),
        )
