...
def make_model(R=1, gamma=0, var=0.3, two_prong=True, prong_pts=(1.0, 1.0)):
    if two_prong:
        ...
        comp = D.Independent(
            D.Normal(
                torch.Tensor(
                    (
                        (R * cos(gamma), R * sin(gamma)),
                        (-R * cos(gamma), -R * sin(gamma)),
                    )
                ),
                torch.Tensor(((var, var), (var, var))),
            ),
            1,
        )
...
def make_TH2(R=1, gamma=0, var=0.3, two_prong=True):
    if two_prong:
        return ROOT.TF2(
            "f",
            ".5/(2*pi*({var}))*(exp(-.5*(x+({R})*cos({gamma}))**2/({var})**2 - .5*(y+({R})*sin({gamma}))**2/({var})**2) + exp(-.5*(x-({R})*cos({gamma}))**2/({var})**2 - .5*(y-({R})*sin({gamma}))**2/({var})**2))".format(
                var=var, R=R, gamma=gamma
            ),
            -2,
            2,
            -2,
        ...
        return ROOT.TF2(
            "f",
            "1./(2*pi*({var}))*exp(-.5*(x-({R})*cos({gamma}))**2/({var})**2 - .5*(y-({R})*sin({gamma}))**2/({var})**2)".format(
                var=var, R=R, gamma=gamma
            ),
            -2,
            2,
...
def sample(model, Nevents):
    ...
    angles = torch.Tensor(
        [
            torch.nn.functional.pad(
                model.sample((Nparticles.sample().int(),)), (0, 0, 0, Nparticle_pad)
            )[:Nparticle_pad].numpy()
    ...
    pt = (
        pt_jet
        * torch.distributions.log_normal.LogNormal(0, 0.2).sample(
            (Nevents, Nparticle_pad)
        )
        * (pt * mask)
        / (pt * mask).sum(dim=-1).reshape(-1, 1)
    )
...
def mix_model(model1, model2, Nevents):
    ...
    Nparticles = D.Poisson(mean_Nparticles / 2.0)
    npart = [
        (Nparticles.sample().int().item(), Nparticles.sample().int().item())
        for _ in range(Nevents)
    ]
        ...
        [
            torch.cat(
                (
                    torch.ones(npart1),
                    -1 * torch.ones(npart2),
                    torch.zeros(max(0, Nparticle_pad - npart1 - npart2)),
                ),
                dim=0,
            )[:Nparticle_pad].numpy()
            for npart1, npart2 in npart
        ]
    )
    angles = torch.Tensor(
        [
            torch.nn.functional.pad(
                torch.cat((model1.sample((npart1,)), model2.sample((npart2,))), dim=0),
                (0, 0, 0, Nparticle_pad),
            )[:Nparticle_pad].numpy()
            for npart1, npart2 in npart
        ]
...
def getEvents(
    signal, background, nTraining, test_size=None, train_size=None, ret_sig_bkg=False
):
    ...
    if ret_sig_bkg:
        return (
            torch.Tensor(pt_sig).to(device),
            torch.Tensor(angles_sig).to(device),
            torch.Tensor(pt_bkg).to(device),
            torch.Tensor(angles_bkg).to(device),
        )
