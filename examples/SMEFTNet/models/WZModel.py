...
selection = (
    lambda ar: (ar.genJet_pt > 500)
    & (ar.dR_genJet_maxq1q2 < 0.6)
    & (ar.genJet_SDmass > 70)
    & (ar.genJet_SDmass < 110)
)
...
def getWeights(eft, coeffs, lin=False):
    ...
    fac = np.array(
        [
            functools.reduce(
                operator.mul,
                [(float(eft[v]) - weightInfo.ref_point_coordinates[v]) for v in comb],
                1,
            )
            for comb in combs
        ],
...
class WZModel:
    def __init__(self, charged=False, scalar_features=[], what="lab", operator="cW"):
        ...
        if scalar_features is not None and len(scalar_features) > 0:
            ...
            if not type(self.scalar_features) == type([]):
                raise RuntimeError("Need a list of scalar features")
        ...
        self.data_generator = DataGenerator(
            input_files=[
                os.path.join(
                    user.data_directory, "v6/WZto2L_HT300_Ref_ext_v4_v2/*.root"
                )
            ],
            n_split=200,
    ...
    def getEvents(self, data):
        ...
        coeffs = DataGenerator.vector_branch(
            data, "p_C", padding_target=len(weightInfo.combinations)
        )
        ...
        if self.charged:
            charge = DataGenerator.vector_branch(
                data, "gen_charge", padding_target=padding
            )  # [ptmask
        ...
        if self.scalar_features:
            scalar_features = torch.Tensor(
                DataGenerator.scalar_branches(data, self.scalar_features)
            ).to(device)
        else:
        ...
        if self.what == "lab":
            detas = torch.Tensor(
                DataGenerator.vector_branch(
                    data, "gen_Deta_lab", padding_target=padding
                )
            ).to(device)
            dphis = torch.Tensor(
                DataGenerator.vector_branch(
                    data, "gen_Dphi_lab", padding_target=padding
                )
            ).to(device)
            ...
            q12_dphi = dphi(
                torch.Tensor(
                    DataGenerator.scalar_branches(data, ["parton_hadV_q2_phi"])
                ).to(device),
                torch.Tensor(
                    DataGenerator.scalar_branches(data, ["parton_hadV_q1_phi"])
                ).to(device),
            )
            q12_deta = torch.Tensor(
                DataGenerator.scalar_branches(data, ["parton_hadV_q2_eta"])
            ).to(device) - torch.Tensor(
                DataGenerator.scalar_branches(data, ["parton_hadV_q1_eta"])
            ).to(
                device
            )
            ...
            thetas = torch.Tensor(
                DataGenerator.vector_branch(
                    data, "gen_Theta_VV", padding_target=padding
                )
            ).to(device)
            dphis = torch.Tensor(
                DataGenerator.vector_branch(data, "gen_phi_VV", padding_target=padding)
            ).to(device)
            angles = torch.stack((thetas * ptmask, dphis * ptmask), axis=2)

            parton_hadV_q1_phi = torch.Tensor(
                DataGenerator.scalar_branches(data, ["parton_hadV_angle_phi"])
            ).to(device)
            parton_hadV_pt = torch.Tensor(
                DataGenerator.scalar_branches(data, ["parton_hadV_pt"])
            ).to(device)
            parton_lepV_pt = torch.Tensor(
                DataGenerator.scalar_branches(data, ["parton_lepV_pt"])
            ).to(device)
            truth = torch.stack(
                [parton_hadV_q1_phi[:, 0], parton_hadV_pt[:, 0], parton_lepV_pt[:, 0]],
                axis=1,
            )
        ...
        return (
            ...
            features,
            scalar_features,
            torch.stack([weight_sm, target], axis=1),
            truth,
        )
...
if __name__ == "__main__":
    ...
    print(
        "Reading all events at once: Got",
        len(model.getEvents(model.data_generator[-1])[0]),
    )
