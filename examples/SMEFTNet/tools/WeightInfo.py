...
class WeightInfo:
    def __init__(self, filename):
        ...
        if "rw_dict" in data.keys():
            self.data = data["rw_dict"]
        else:
            self.data = data

        if "order" in data.keys():
            self.pkl_order = data["order"]["order"]
        else:
            self.pkl_order = None

        if "ref_point" in data.keys():
        ...
        self.ref_point_coordinates = {
            var: (
                float(self.ref_point[var])
                if (self.ref_point is not None and var in self.ref_point.keys())
                else 0
            )
        ...
        logger.debug(
            "Found %i variables: %s. Found %i weights."
            % (self.nvar, ",".join(self.variables), self.nid)
        )
    ...
    def set_order(self, order):
        if self.pkl_order == None:
            ...
            raise ValueError(
                "Polynomial order is greater than in the gridpack (order %i)"
                % self.pkl_order
            )
        self.order = order
    ...
    def get_ndof(nvar, order):
        return sum(
            [int(scipy.special.binom(nvar + o - 1, o)) for o in range(order + 1)]
        )
    ...
    def make_combinations(variables, order):
        ...
        for o in range(order + 1):
            combinations.extend(
                list(itertools.combinations_with_replacement(variables, o))
            )
        return combinations
        ...
        for i_comb, comb in enumerate(self.combinations):
            ...
            for v in comb:
                if self.ref_point_coordinates[v] == 0:
                    ...
                    subsubstrings.append(
                        "(rw_%s-%s)"
                        % (v, str(self.ref_point_coordinates[v]).rstrip("0"))
                    )
    ...
    def set_default_args(self, args):
        ...
        if len(unused_args) > 0:
            raise ValueError(
                "Variable %s not in the gridpack! Please use only the following variables: %s"
                % (" && ".join(unused_args), ", ".join(self.variables))
            )
    ...
    def get_weight_string(self, **kwargs):
        ...
        substrings = []
        for i_comb, comb in enumerate(self.combinations):
            if False in [v in kwargs for v in comb]:
                continue
            # remove 0 entries
            fac = float(
                reduce(
                    mul,
                    [(float(kwargs[v]) - self.ref_point_coordinates[v]) for v in comb],
                    1,
                )
            )
            if abs(fac) == 0.0:
    ...
    def getCoeffListFromDraw(self, sample, selectionString, weightString=None):
        ...
        histo = sample.get1DHistoFromDraw(
            ...
            weightString=(
                "p_C*(%s)" % weightString if weightString is not None else "p_C"
            ),
        )
    ...
    def getCoeffPlotFromDraw(
        ...
        if nEventsThresh > 0:
            histEntries = sample.get1DHistoFromDraw(
                variableString,
                binning,
                selectionString=selectionString,
                weightString="(1)",
            ...
            return [
                (
                    histo_to_list(histo.ProjectionX("%i_px" % i, i + 1, i + 1))
                    if int(nBinEvents) >= int(nEventsThresh)
                    else [0 for j in range(histo.GetNbinsX())]
                )
                for i, nBinEvents in enumerate(numEntries)
            ]
        else:
            return [
                histo_to_list(histo.ProjectionX("%i_px" % i, i + 1, i + 1))
                for i in range(histo.GetNbinsY())
            ]

    # Make a coeff histo from a sample
    def get2DCoeffPlotFromDraw(
        self,
        sample,
        variableString,
        binning,
        ...
        # works without if as well, but saves time
        if nEventsThresh > 0:
            histEntries = sample.get2DHistoFromDraw(
                variableString,
                binning,
                selectionString=selectionString,
                weightString="(1)",
            ...
            numEntries = [
                histo_to_list(histEntries.ProjectionY("%i_py" % j, j + 1, j + 1))
                for j in range(histEntries.GetNbinsX())
            ]
            return [
                histo_to_list(
                    histo.ProjectionX("%i_%i_px" % (i, j), i + 1, i + 1, j + 1, j + 1)
                )
                for i, nEventsList in enumerate(numEntries)
                for j, nBinEvents in enumerate(nEventsList)
                if int(nBinEvents) >= int(nEventsThresh)
            ]
        else:
            return [
                ...
                for i in range(histo.GetNbinsY())
                for j in range(histo.GetNbinsZ())
            ]

    # Make a coeff histo from a sample
    def get3DCoeffPlotFromDraw(
        self,
        sample,
        variableString,
        binning,
        ...
        if len(binning) != 9:
            raise ValueError(
                "Binning has to be in the format [bins1, min1, max1, bins2, min2, max2, bins3, min3, max3]!"
            )
        ...
        for i, bound in enumerate(bounds[1:]):
            ...
            coeffList3D.append(
                self.get2DCoeffPlotFromDraw(
                    sample,
                    variableString2D,
                    binning[:6],
                    selectionString
                    + "&&%s>=%f&&%s<%f"
                    % (variableString3D, bounds[i], variableString3D, bound),
                    weightString,
                    nEventsThresh,
                )
            )
    ...
    def getCoeffListFromEvents(sample, selectionString=None, weightFunction=None):
        ...
        while reader.run():
            coeffs.append(
                [
                    (
                        reader.event.p_C[i] * (weightFunction(reader.event, sample))
                        if weightFunction is not None
                        else reader.event.p_C[i]
                    )
                    for i in range(reader.event.np)
                ]
            )
    ...
    def getFisherInformationHisto(
        ...
        **kwargs,
        ...
        sample.setSelectionString("1")
        coeffList = self.getCoeffPlotFromDraw(
            sample,
            variableString,
            binning,
            selectionString,
            weightString=weightString,
            nEventsThresh=nEventsThresh,
        )
        detIList = [
            (
                np.linalg.det(
                    self.get_fisherInformation_matrix(coeffs, variables, **kwargs)[1]
                )
                if len(coeffs) != 0 and not all([v == 0 for v in coeffList])
                else 0
            )
    ...
    def get_diff_weight_string(self, var, **kwargs):
        ...
        if type(var) == type("") and var not in self.variables:
            raise ValueError(
                "Variable %s not in list of variables %r" % (var, self.variables)
            )
        if type(var) == tuple and not all(v in self.variables for v in var):
            raise ValueError(
                "One variable of %r not in list of variables %r" % (var, self.variables)
            )
    ...
    def get_fisher_weight_string(self, var1, var2, **kwargs):
        ...
        if var1 == var2:
            return "(%s)**2/(%s)" % (
                self.get_diff_weight_string(var1, **kwargs),
                self.get_weight_string(**kwargs),
            )
        else:
            return "(%s)*(%s)/(%s)" % (
                self.get_diff_weight_string(var1, **kwargs),
                self.get_diff_weight_string(var2, **kwargs),
                self.get_weight_string(**kwargs),
            )
    ...
    def get_weight_func(self, **kwargs):
        ...
        return lambda event, sample, terms=terms: sum(
            event.p_C[term[0]] * term[1] for term in terms
        )
    ...
    def get_diff_weight_yield(self, vars, coeffList, **kwargs):
        ...
        for var in vars:
            if var not in self.variables:
                raise ValueError(
                    "Variable %s not in gridpack: %r" % (var, self.variables)
                )
    ...
    def get_fisherInformation_matrix(self, coeffList, variables=None, **kwargs):
        ...
        # check if coeffList is filled with 0
        if all([v == 0 for v in coeffList]):
            return variables, np.zeros((len(variables), len(variables)))
        ...
        diff_weight_yield = {
            var: self.get_diff_weight_yield(var, coeffList, **kwargs)
        ...
        fi_matrix = np.full(
            ...
            1.0 / weight_yield if weight_yield != 0 else 0,
        )
        ...
        for i, var_i in enumerate(variables):
            for j, var_j in enumerate(variables):
                ...
                if i <= j:
                    fi_matrix[i, j] *= (
                        diff_weight_yield[var_i] * diff_weight_yield[var_j]
                    )
                else:
    ...
    def get_total_fisherInformation_matrix(self, coeffLists, variables=None, **kwargs):
        ...
        fi_matrix = np.sum(
            [
                self.get_fisherInformation_matrix(coeffList, variables, **kwargs)[1]
                for coeffList in coeffLists
                if not all([v == 0 for v in coeffList])
            ],
            0,
        )
    ...
    def get_christoffels(self, coeffLists, variables=None):
        ...
        def christoffel_symbols(index, position):
            ...
            metric = self.get_total_fisherInformation_matrix(
                coeffLists, variables=_variables, **kwargs_
            )[1]
            ...
            for coeffList in coeffLists:
                ...
                diff_weight_yield = {
                    i_var: self.get_diff_weight_yield(var, coeffList, **kwargs_)
                    for i_var, var in enumerate(_variables)
                }
                diff2_weight_yield = {
                    (i_var_1, i_var_2): self.get_diff_weight_yield(
                        (var_1, var_2), coeffList, **kwargs_
                    )
                    for i_var_1, var_1 in enumerate(_variables)
                    for i_var_2, var_2 in enumerate(_variables)
                }
