...
class DataGenerator(Sequence):
    ...
    def reduceFiles(self, to):
        ...
        print(
            "Reducing files to %i. n_split: %i" % (len(self.input_files), self.n_split)
        )
    ...
    def _load(self, index, small=None):
        ...
        if self.verbose:
            print(
                "Loading index %i, strategy: %s"
                % (index, self.splitting_strategy.lower())
            )
        if index >= 0:
        ...
        if self.splitting_strategy.lower() == "files":
            ...
            self.array = uproot.concatenate(
                [
                    f + ":" + self.tree_name
                    for f in self.input_files[filestart:filestop]
                ],
                self.branches,
            )
            if self.selection is not None:
                ...
                if self.verbose:
                    print(
                        "Applying selection with efficiency %4.3f"
                        % (len(self.array) / len_before)
                    )
            entry_start, entry_stop = 0, len(self.array)
        elif self.splitting_strategy.lower() == "events":
            if not hasattr(self, "array"):
                self.array = uproot.concatenate(
                    [f + ":" + self.tree_name for f in self.input_files], self.branches
                )
                if self.selection is not None:
                    len_before = len(self.array)
                    self.array = self.array[self.selection(self.array)]
    ...
    @staticmethod
    def vector_branch(data, branches, padding_target=50, padding_value=0.0):
        if type(branches) == str:
            return np.array(
                ak.fill_none(
                    ak.pad_none(
                        data[branches].to_list(), target=padding_target, clip=True
                    ),
                    value=padding_value,
                )
            )
            ...
            return np.array(
                [
                    np.array(
                        ak.fill_none(
                            ak.pad_none(
                                data[b].to_list(), target=padding_target, clip=True
                            ),
                            value=padding_value,
                        )
                    ).transpose()
