...
def write_sync_files_txt(output_filename="file_sync_storage.txt"):
    ...
    for filename in list(set(file_sync_storage)):
        ...
        phpfiles = [
            ...
            if os.path.isfile(os.path.join(dir_path, f)) and f.endswith(".php")
        ]
        for f in phpfiles:
    ...
    if len(file_sync_storage) > 0:
        ...
        print(
            (
                "Analysis.Tools.syncer: Written %i files to %s for rsync."
                % (n_files, output_filename)
            )
        )
    return n_files
...
def makeRemoteGif(directory, pattern, name, delay=50):
    ...
    remotedir = "/eos/user/{INITIAL}/{CERN_USER}/www/{directory}".format(
        CERN_USER=cern_user, INITIAL=cern_user[0], directory=directory_
    )
    cmd = "ssh {CERN_USER}@lxplus.cern.ch 'convert -delay {delay} -loop 0 {remotedir}/{pattern} {remotedir}/{name}.gif'".format(
        CERN_USER=cern_user,
        delay=delay,
        remotedir=remotedir,
        pattern=pattern,
...
def sync(gifs=False):
    ...
    cmd = (
        "rsync -avR  `cat %s` ${CERN_USER}@lxplus.cern.ch:/eos/user/$(echo ${CERN_USER} | head -c 1)/${CERN_USER}/www/"
        % filename
    )
    print(cmd)
    output, error = subprocess.Popen(
