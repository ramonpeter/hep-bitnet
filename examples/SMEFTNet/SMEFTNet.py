...
from bitnet.bitlinear import BitLinear158b
...
class EdgeConv(MessagePassing):
    ...
    def message(self, x_i, x_j):
        ...
        norm = torch.sqrt(
            (x_i[:, -2] ** 2 + x_i[:, -1] ** 2) * (x_j[:, -2] ** 2 + x_j[:, -1] ** 2)
        )
        cos_ij, sin_ij = (
            (x_i[:, -2] * x_j[:, -2] + x_i[:, -1] * x_j[:, -1]) / norm,
            (x_i[:, -1] * x_j[:, -2] - x_i[:, -2] * x_j[:, -1]) / norm,
        )
        mlp = self.mlp(
            torch.cat(  # .. and mlp output as fkt of
                [
                    ...
                    cos_ij.view(
                        -1, 1
                    ),  # and two angular coordinates, in / cos(gamma_i - gamma_j)
        ...
        if torch.any(torch.isnan(mlp)):
            print(
                "Warning! Found nan in message passing MLP output. Set to zero (likely zero variance)."
            )
    ...
    def aggregate(self, inputs, index):  # ,  **kwargs):
        ...
        wj = pt / (
            torch.zeros_like(index.unique(), dtype=torch.float).index_add_(
                0, index, pt
            )[index]
        )
        if torch.any(torch.isnan(wj)):
            print(
                "Warning! Found nan in pt weighted message passing (aggregation). There is a particle with only pt=0 particles in its neighbourhood. Replace with zero."
            )
            wj = torch.nan_to_num(wj)

        # first, weight ALL inputs
        result = (
            torch.zeros((len(index.unique()), inputs.shape[1]), dtype=torch.float)
            .to(device)
            .index_add_(0, index, wj.view(-1, 1) * inputs)
        )
        ...
        result = torch.cat(
            (
                result[:, :-3],
                torch.view_as_real(
                    torch.exp(2 * torch.pi * 1j * result[:, -3])
                    * torch.view_as_complex(result[:, -2:].contiguous())
                ),
            ),
            dim=1,
        )
        ...
        with torch.no_grad():
            if self.message_logging:
                self.message_dict["message"] = torch.sqrt(
                    torch.square(inputs[:, 1:-3]).sum(dim=-1)
                ).numpy()
...
class EIRCGNN(EdgeConv):
    ...
    def forward(self, x, batch):
        ...
        if (
            self.include_features_in_radius is not None
            and len(self.include_features_in_radius) > 0
        ):
            features = x[
                :, 2:-2
            ]  # We start taking from "2:" because the first feature in the first EIRCGNN is rho
            x_radius = torch.stack(
                [x[:, -2], x[:, -1]]
                + [features[:, pos] for pos in self.include_features_in_radius]
            ).transpose(0, 1)
        else:
            x_radius = x[:, -2:]
...
class SMEFTNet(torch.nn.Module):
    def __init__(
        ...
        self.scalar_batch_norm = (
            torch.nn.BatchNorm1d(num_scalar_features)
            if (num_scalar_features > 0 and scalar_batch_norm)
            else None
        )
        # tuple index of features that should be included in computing the distance
        self.include_features_in_radius = include_features_in_radius
        self.readout_batch_norm = readout_batch_norm
        if self.readout_batch_norm and self.scalar_batch_norm:
            print("Warning! Two batch norms for scalar features!")
        ...
        for l, (dropout, hidden_layers) in enumerate(conv_params):
            ...
            if l == 0:
                _mlp = MLP(
                    [3 * (1 + num_features) + 2] + hidden_layers_,
                    dropout=dropout,
                ...
                _mlp = MLP(
                    [3 * conv_params[l - 1][1][-1] + 2] + hidden_layers_,
                    dropout=dropout,
                    activation="leakyrelu",
                )
                # _mlp.act.negative_slope = negative_slope
        ...
        self.mlp = MLP(
            [EC_out_chn + self.num_scalar_features] + readout_params[1] + [num_classes],
            dropout=readout_params[0],
            activation="leakyrelu",
            batch_norm=self.readout_batch_norm,
        )
    ...
    def load(cls, directory, epoch=None):
        ...
        cfg_dict = pickle.load(
            open(load_file_name.replace("_state.pt", "_cfg_dict.pkl"), "rb")
        )
        model = cls(
            num_classes=cfg_dict["num_classes"] if "num_classes" in cfg_dict else 1,
            conv_params=eval(cfg_dict["conv_params"]),
            dRN=cfg_dict["dRN"],
            readout_params=eval(cfg_dict["readout_params"]),
            learn_from_gamma=(
                cfg_dict["learn_from_gamma"]
                if "learn_from_gamma" in cfg_dict
                else cfg_dict["learn_from_phi"]
            ),
        )
    ...
    def forward(
        ...
        if len(self.EC) > 0:
            ...
            if features is not None:
                assert (
                    features.shape[2] == self.num_features
                ), "Got %i features but was expecting %i." % (
                    features.shape[2],
                    self.num_features,
                )
                x = torch.cat(
                    (
                        pt[mask].view(-1, 1),
                        torch.view_as_complex(angles[mask]).abs().view(-1, 1),
                        features[mask],
                        angles[mask],
                    ),
                    dim=1,
            ...
            wj = (
                pt
                / (
                    torch.zeros_like(batch.unique(), dtype=torch.float).index_add_(
                        0, batch, pt
                    )
                )[batch]
            )
            if torch.any(torch.isnan(wj)):
                print(
                    "Warning! Found nan in pt weighted readout. Are there no particles with pt>0?. Replace with zero."
                )
                wj = torch.nan_to_num(wj)

            # disregard first column (pt, keep the last two ones: cos/sin gamma)
            x = (
                torch.zeros((len(batch.unique()), x[:, 1:].shape[1]), dtype=torch.float)
                .to(device)
                .index_add_(0, batch, wj.view(-1, 1) * x[:, 1:])
            )
        ...
        if scalar_features is not None:
            y = (
                self.scalar_batch_norm(scalar_features)
                if self.scalar_batch_norm is not None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--learning_rate", "--lr", action="store", default=0.001, help="Learning rate"
    )
    parser.add_argument(
        "--learn_from_gamma", action="store_true", help="SMEFTNet parameter"
    )
    parser.add_argument(
        "--epochs", action="store", default=100, type=int, help="Number of epochs."
    )
    parser.add_argument(
        "--nTraining", action="store", default=1000, type=int, help="Number of epochs."
    )
    ...
    pt_train, pt_test, angles_train, angles_test, labels_train, labels_test = (
        train_test_split(
    ...
    model_directory = os.path.dirname(
        os.path.join(user.model_directory, "EIRCGNN", args.prefix)
    )
    ...
    if not args.overwrite:
        files = glob.glob(
            os.path.join(
                user.model_directory, "EIRCGNN", args.prefix + "_epoch-*_state.pt"
            )
        )
        if len(files) > 0:
            load_file_name = max(
                files, key=lambda f: int(f.split("-")[-1].split("_")[0])
            )
    ...
    scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=1.0, end_factor=1.0 / 20
    )

    criterion = torch.nn.BCELoss()

    pt_train, pt_test, angles_train, angles_test, labels_train, labels_test = getEvents(
        args.nTraining
    )
    for epoch in range(epoch_min, args.epochs):
        ...
        if args.prefix:
            torch.save(
                ...
                os.path.join(
                    ...
                    "EIRCGNN",
                    args.prefix + "_epoch-%d_state.pt" % epoch,
                ),
            )
            torch.save(
                ...
                os.path.join(
                    user.model_directory,
                    "EIRCGNN",
                    args.prefix + "_epoch-%d_optimizer.pt" % epoch,
                ),
            )
        ...
        print(
            f"Epoch {epoch:03d} with N={n_samples:03d}, Loss(train): {loss:.4f} Loss(test): {loss_test:.4f}"
        )
